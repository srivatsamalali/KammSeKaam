...
const ChatThreadPanel = ({ isOpen, onClose, applicationId, candidateName, currentUser }) => {
  ...
  return (
    <div className={`fixed bottom-6 right-6 z-[9999] flex flex-col w-[360px] h-[520px] max-h-[80vh] bg-white dark:bg-slate-900 border border-slate-205 dark:border-slate-800 rounded-2xl shadow-2xl overflow-hidden transition-all duration-300 ease-in-out ${
      animate ? 'opacity-100 translate-y-0 scale-100 pointer-events-auto' : 'opacity-0 translate-y-4 scale-95 pointer-events-none'
    }`}>
      ...
      <div className="p-4 border-b border-slate-100 dark:border-slate-850 flex justify-between items-center bg-slate-50/50 dark:bg-slate-900/50">
        <div>
          ...
          <p className="text-[10px] text-slate-500">
            Candidate: <span className="font-semibold text-amber-750 dark:text-amber-500">{candidateName}</span>
          </p>
        </div>
        <button 
          onClick={onClose}
          className="p-1.5 rounded-full hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-400 hover:text-slate-700 transition-colors focus:outline-hidden"
        >
      ...
      <form onSubmit={handleSend} className="p-3 border-t border-slate-100 dark:border-slate-850 bg-white dark:bg-slate-900 flex gap-2 items-center">
        <input 
          ...
          disabled={loading}
          className="form-input text-xs h-9 py-1 flex-1 bg-slate-50 border-slate-200 dark:bg-slate-950 dark:border-slate-850 focus:outline-hidden"
        />
        <button 
          type="submit"
          disabled={loading || !inputText.trim()}
          className="btn-primary text-xs h-9 px-4 shrink-0 font-bold flex items-center justify-center gap-1.5 focus:outline-hidden"
        >
          <span>Send</span>
